...
- The frontend product IDs (currently strings like `"elec-001"`) will be resolved to backend integer product IDs using an existing lookup mechanism (slug-based product retrieval or an ID mapping approach). The resolution strategy is an implementation detail left to planning.
...
1. **Given** a user has just placed an order, **When** they are redirected to the confirmation page, **Then** the page fetches and displays the order number, ordered items (names, quantities, prices), total amount, and shipping address from the backend.
2. **Given** a user is on the confirmation page, **When** the order details are loading, **Then** the page shows a loading indicator until the data is ready.
...
- **FR-002**: System MUST resolve frontend product identifiers (strings) to backend product identifiers (integers) before submitting the order, ensuring each item references a valid backend product.
- **FR-003**: System MUST include the unit price from the product catalog for each line item in the order request.
- **FR-004**: System MUST calculate the order total as the sum of (quantity x unit price) for all items, and this total MUST match the backend's expected total within a rounding tolerance.
...
- **FR-008**: System MUST display a user-friendly error message when the backend returns a validation error (e.g., invalid product, total mismatch) and MUST NOT clear the cart.
- **FR-009**: System MUST display a user-friendly error message when a network failure or server error occurs during order submission and MUST NOT clear the cart.
- **FR-010**: System MUST re-enable the "Place Order" button after an error so the user can retry.
- **FR-011**: The order confirmation page MUST fetch and display real order details (order number, items, totals, shipping address) from the backend using the order number.
- **FR-012**: The order confirmation page MUST show a loading state while fetching order details.
...
- **Order**: A confirmed purchase record containing a unique order number (format: `ORD-YYYY-XXXXX`), customer information (name, email), shipping address, total amount, order status, and creation timestamp. Each order has one or more order items.
